  ...
  ZPAccessory.prototype.setPlayOn = function(on, callback) {
    ...
    if (on) {
      ...
      if (this.state.group.on) {
        this.log.info('%s: already playing', this.name);
        return callback();
      }
      ...
      return this.setZoneOn(false, function() {
        this.state.zone.on = false;
        this.log.info('%s: left old group to find new group', this.name);
        ...
        for (const id in this.platform.zpAccessories) {
          ...
          if (accessory.state.group.on && accessory.isCoordinator && accessory.state.group.track === 'TV') {
            this.log.info('%s: found new coordinator (TV) -> ', this.name, accessory.name);
        ...
        for (const id in this.platform.zpAccessories) {
          ...
          if (accessory.state.group.on && accessory.isCoordinator) {
            this.log.info('%s: found new coordinator -> ', this.name, accessory.name);
            coordinator = accessory;
            break;
          }
        }
      ...
      // Leaves any group
      var isCoordinator = this.isCoordinator;
      return this.setZoneOn(false, function() {
        this.state.zone.on = false;
        this.log.info('%s: left old group to stop playback', this.name);

        // Stops playback
        if (isCoordinator) {
          this.log.info('%s: stop playback as coordinator', this.name);
          return this.setGroupOn(false, function() {
            this.state.group.on = false;
            this.groupService.setCharacteristic(Characteristic.On, false);
            this.state.group.on = false;
            this.zoneService.setCharacteristic(Characteristic.On, false);
            this.state.zone.on = false;
            return callback();
          }.bind(this));
        } else {
