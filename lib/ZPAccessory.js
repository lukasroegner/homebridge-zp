...
ZPAccessory.prototype.handleAVTransportEvent = function(data) {
  ...
  ZPAccessory.prototype.setPlayOn = function(on, callback) {
    ...
    if (on && this.state.group.on) {
      this.log.info('%s: play state is already on', this.name);
      return callback();
    }

    // Defines the function that is called after the device has left any group
    var afterLeavingGroup = function() {

      // Checks if the play state should be set to off
      if (!on) {
        if (!this.state.group.on) {
          this.log.info('%s: finished setting play state as group is already off', this.name);
          return callback();
        } else {
          this.log.info('%s: setting play state of group to off', this.name);

          // Defines the function that waits for the new group state
          var waitForAfterPausingGroup = function() {
            this.log.info('%s: waiting for confirmation that the new group state is false', this.name);
            if (!this.state.group.on) {
              this.log.info('%s: group state confirmed', this.name);
              return callback();
            } else {
              return setTimeout(waitForAfterPausingGroup, 500);
            }
          }.bind(this);

          // Stops the playback
          return this.setGroupOn(false, waitForAfterPausingGroup);
        }
      } else {
        ...
        for (const id in this.platform.zpAccessories) {
          ...
          if (accessory.state.group.on && accessory.isCoordinator && accessory.state.group.track === 'TV') {
            this.log.info('%s: found new coordinator (TV) %s', this.name, accessory.name);
        ...
        for (const id in this.platform.zpAccessories) {
          ...
          if (accessory.state.group.on && accessory.isCoordinator) {
            this.log.info('%s: found new coordinator %s', this.name, accessory.name);
            coordinator = accessory;
            break;
          }
        }
        ...
        // Defines the function that waits for the new group state
        var waitForAfterPlayingGroup = function() {
          this.log.info('%s: waiting for confirmation that the new group state is true', this.name);
          if (this.state.group.on) {
            this.log.info('%s: group state confirmed', this.name);
            return callback();
          } else {
            return setTimeout(waitForAfterPlayingGroup, 500);
          }
        }.bind(this);

        // Starts playback of current group
        this.log.info('%s: settings play state of own group as no other coordinator found', this.name);
        return this.setGroupOn(true, waitForAfterPlayingGroup);
      }
    }.bind(this);

    // Defines the function that waits for the new zone state
    var waitForAfterLeavingGroup = function() {
      this.log.info('%s: waiting for confirmation that any group has been left', this.name);
      if (!this.state.zone.on) {
        return afterLeavingGroup();
      } else {
        setTimeout(waitForAfterLeavingGroup, 500);
      }
    }.bind(this);

    // Leaves any group
    if (this.state.zone.on) {
      this.log.info('%s: leaving any group to prepare new play state', this.name);
      return this.setZoneOn(false, waitForAfterLeavingGroup);
    } else {
      this.log.info('%s: leaving any group not neccessary to prepare new play state', this.name);
      return afterLeavingGroup();
    }
  };
