...
function ZPAccessory(platform, zp) {
  ...
  this.playService = new Service.Outlet(this.zp.zone + ' Player', 'player');
  this.playService.getCharacteristic(Characteristic.On)
    .on('set', this.setPlayOn.bind(this));
  this.playService.setCharacteristic(Characteristic.OutletInUse, 1);
  if (this.platform.players) {
    this.services.push(this.playService);
  }

  this.nightModeService = new Service.Outlet(this.zp.zone + ' Night Mode', 'nightmode');
  this.nightModeService.getCharacteristic(Characteristic.On)
    .on('set', this.setNightModeOn.bind(this));
  if (this.tv) {
    this.services.push(this.nightModeService);
  }

  this.speechEnhancementService = new Service.Outlet(this.zp.zone + ' Speech Enhancement', 'speechenhancement');
  this.speechEnhancementService.getCharacteristic(Characteristic.On)
    .on('set', this.setSpeechEnhancementOn.bind(this));
  if (this.tv) {
    this.services.push(this.speechEnhancementService);
  }
  ...
  if (this.platform.volume) {
    const volume = this.platform.volume[this.zone];
    if (volume) {
      setInterval(() => {
        const currentTimeOfDay = (new Date().getHours() * 3600) + (new Date().getMinutes() * 60) + (new Date().getSeconds());
        for (let timeOfDayString in volume) {
          let timeOfDay = parseInt(timeOfDayString);
          if (currentTimeOfDay >= timeOfDay - 120 && currentTimeOfDay <= timeOfDay + 120) {
            this.setZoneVolume(volume[timeOfDayString], function() {});
          }
        }
      }, 30 * 1000);
    }
...
ZPAccessory.prototype.copyCoordinator = function() {
  ...
  if (coordinator && coordinator !== this) {
    ...
    if (this.state.group.on !== coordinator.state.group.on) {
      ...
      this.playService.updateCharacteristic(Characteristic.On, this.state.group.on);
    }
  ...
  this.parser.parseString(data.LastChange, function(err, json) {
    ...
    if (event.NightMode) {
      ...
      if (nightSound !== this.state.zone.nightSound) {
        ...
        if (this.tv) {
          this.nightModeService.setCharacteristic(Characteristic.On, this.state.zone.nightSound);
        }
      }
    }
    if (event.DialogLevel) {
      ...
      if (speechEnhancement !== this.state.zone.speechEnhancement) {
        ...
        if (this.tv) {
          this.speechEnhancementService.setCharacteristic(Characteristic.On, this.state.zone.speechEnhancement);
        }
      }
    }
  }.bind(this));
...
ZPAccessory.prototype.setNightModeOn = function (nightSound, callback) {
...
// Called by homebridge when characteristic is changed from homekit.
ZPAccessory.prototype.setSpeechEnhancementOn = function (speechEnhancement, callback) {
...
// Is called when the play state is set to on
ZPAccessory.prototype.setPlayOn = function(on, callback) {
  this.log.info('%s: set play state to %s', this.name, on);
  
  // If the group is already playing, do nothing
  if (on && this.state.group.on) {
    this.log.info('%s: play state is already on, finished', this.name);
    return callback();
  }

  // If the zone is the coordinator of a TV source, do nothing
  if (this.isCoordinator && this.state.group.track === "TV" && this.state.group.on) {
    this.log.info('%s: zone is coordinator of TV, finished', this.name);
    return callback();
  }

  // Defines the function that is called after any group has been left
  var leftGroup = function() {
    this.log.info('%s: group left', this.name);

    // Defines the function that waits for the zone to be coordinator (group has been left)

    var waitForBecomeCoordinator = function(retryCount) {
      if (!this.isCoordinator && retryCount < 5) {
        this.log.info('%s: wait for becoming coordinator', this.name);
        retryCount += 1;
        return setTimeout(function() { waitForBecomeCoordinator(retryCount); }, 500);
      } else {
        this.log.info('%s: became coordinator', this.name);

        // Pauses the playback
        if (!on) {
          return this.zp.pause(function(err, success) {
            this.log.info('%s: play state set to false, finished', this.name);
            this.state.group.on = false;
            if (this.showGroups) {
              this.groupService.setCharacteristic(Characteristic.On, this.state.group.on);
            }
            this.playService.updateCharacteristic(Characteristic.On, this.state.group.on);
        ...
        // Initializes the new coordinator for the player
        var coordinator = null;

        // Creates the accessory list
        let remainingAccessories = [];
        for (let id in this.platform.zpAccessories) {
          remainingAccessories.push(this.platform.zpAccessories[id]);
        }

        // Creates the list based on the priorization
        let prioritizedAccessories = [];
        for (let i = 0; i < this.platform.playerZonePriority.length; i++) {
          const zone = this.platform.playerZonePriority[i];
          const filteredList = remainingAccessories.filter(x => x.zone == zone);
          if (filteredList.length > 0) {
            prioritizedAccessories.push(filteredList[0]);
            remainingAccessories.splice(remainingAccessories.indexOf(filteredList[0]), 1);
          }
        }
        
        // Addds the remaining accessories
        while (remainingAccessories.length > 0) {
          prioritizedAccessories.push(remainingAccessories[0]);
          remainingAccessories.splice(0, 1);
        }

        if (this.platform.playerTVPriority) {

          // Tries to get a coordinator that plays sound from TV and is currently on
          for (let i = 0; i < prioritizedAccessories.length; i++) {
            const accessory = prioritizedAccessories[i];
            this.log.info('%s: searching for coordinator (TV), %s with group state %, coordinator state %s and track %s', this.name, accessory.name, accessory.state.group.on, accessory.isCoordinator, accessory.state.group.track);
            if (accessory.state.group.on && accessory.isCoordinator && accessory.state.group.track === 'TV') {
              this.log.info('%s: found new coordinator (TV) %s, finished', this.name, accessory.name);
              coordinator = accessory;
              break;
            }
          }

          // Checks whether a coordinator has been found
          if (coordinator) {
            return this.join(coordinator, callback);
          }
        }

        // Tries to get a coordinator that plays any track and is currently on
          for (let i = 0; i < prioritizedAccessories.length; i++) {
            const accessory = prioritizedAccessories[i];
          this.log.info('%s: searching for coordinator, %s with group state %s and coordinator state %s', this.name, accessory.name, accessory.state.group.on, accessory.isCoordinator);
          if (accessory.state.group.on && accessory.isCoordinator) {
            this.log.info('%s: found new coordinator %s, finished', this.name, accessory.name);
            coordinator = accessory;
            break;
          }
        }
        ...
        // Starts playback of current group
        this.log.info('%s: playing current group, finished', this.name);
        this.zp.play(function(err, success) {
          return callback();
        }.bind(this));
      }
    }.bind(this);
    waitForBecomeCoordinator(0);
  }.bind(this);
  ...
  if (this.isCoordinator) {
    const newCoordinator = this.members()[0];
    if (newCoordinator) {
      var groupOn = this.state.group.on;
      var groupTrack = this.state.group.track;
      newCoordinator.becomePlatformCoordinator();
      return this.abandon(newCoordinator, leftGroup);
    }
    return leftGroup();
  }
  return this.leave(leftGroup);
};
