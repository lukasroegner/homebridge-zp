...
function ZPAccessory(platform, zp) {
  ...
  this.playService = new this.platform.service(this.zp.zone + ' Player', 'player');
  this.playService.addOptionalCharacteristic(Characteristic.Mute);
  this.playService.setCharacteristic(Characteristic.Mute, true);
  this.playService.getCharacteristic(Characteristic.Mute)
...
ZPAccessory.prototype.getServices = function() {
  if (this.platform.speakers) {
    return [this.groupService, this.zoneService, this.playService, this.infoService];
  }
  return [this.groupService, this.infoService, this.playService];
};
  ...
  ZPAccessory.prototype.setPlayOn = function(on, callback) {
    ...
    if (this.isCoordinator) {
      ...
      if (newCoordinator) {
        ...
        newCoordinator.becomePlatformCoordinator();
        return this.abandon(newCoordinator, function() {
          newCoordinator.state.group.on = groupOn;
          newCoordinator.groupService.setCharacteristic(Characteristic.On, newCoordinator.state.group.on);
          newCoordinator.state.group.track = groupTrack;
          newCoordinator.groupService.setCharacteristic(Characteristic.CurrentTrack, newCoordinator.state.group.track);
          leftGroup();
        }.bind(this));
      }
      return leftGroup();
