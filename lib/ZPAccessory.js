...
ZPAccessory.prototype.handleAVTransportEvent = function(data) {
  ...
  ZPAccessory.prototype.setPlayOn = function(on, callback) {
    ...
    var afterLeavingGroup = function() {
      this.log.info('%s: group left for preparing new play state, new zone state is %s', this.name, this.state.zone.on);
      ...
      if (!on) {
        if (!this.state.group.on) {
          ...
          return this.setGroupOn(false, function() {
            this.log.info('%s: play state of group has been set, new group state is %s', this.name, this.state.group.on);
            return callback();
          }.bind(this));
        }
      } else {
        ...
        return this.setGroupOn(true, function() {
          this.log.info('%s: play state of own group has been set, new group state is %s', this.name, this.state.group.on);
          return callback();
        }.bind(this));
      }
    }.bind(this);
    ...
    if (this.state.zone.on) {
      ...
      return this.setZoneOn(false, afterLeavingGroup);
    } else {
