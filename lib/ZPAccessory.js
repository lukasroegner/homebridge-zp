...
ZPAccessory.prototype.handleAVTransportEvent = function(data) {
  ...
  ZPAccessory.prototype.setPlayOn = function(on, callback) {
    ...
    var afterLeavingGroup = function(leftGroup) {
    ...
    if (this.state.zone.on) {
      oldGroupName = this.state.group.name;
      this.log.info('%s: leaving any group (%s) to prepare new play state', this.name, this.state.group.name);
      return this.setZoneOn(false, function() { afterLeavingGroup(true) }.bind(this));
    } else {
      this.log.info('%s: leaving any group not neccessary to prepare new play state', this.name);
      return afterLeavingGroup(false);
    }
  };
