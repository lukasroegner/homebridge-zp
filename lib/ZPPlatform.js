...
function ZPPlatform(log, config) {
  ...
  this.groups = config.groups || false;
  this.players = config.players || false;
  this.playerTVPriority = config.playerTVPriority || false;
  this.playerZonePriority = config.playerZonePriority || [];
  ...
  this.volume = {};
  if (config.volume) {
    for (let zone in config.volume) {
      if (config.volume[zone]) {
        this.volume[zone] = {}
        for (let timeOfDay in config.volume[zone]) {
          const split = timeOfDay.split(':');
          this.volume[zone][(parseInt(split[2]) + (parseInt(split[1]) * 60) + (parseInt(split[0]) * 3600)).toString()] = config.volume[zone][timeOfDay]
        }
      }
    }
  }
