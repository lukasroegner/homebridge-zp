...
Added a new "Player" service, which has a single switch (Characteristic.On). When setting this switch to "on", the Sonos speaker leaves the current group (if any) and joins a new group in the house, that is currently playing (TV players are prioritized). When settings this switch to "off", the speaker leaves the current group (if any) and stops playing.