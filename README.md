...
`groups` | `false` | Flag whether to expose the group to HomeKit. This might be useful when the controls are not required and the Sonos system is used with the Apple Home app.
`players` | `false` | Flag whether to expose the player to HomeKit. A player is a simple outlet and can thus be used in the Apple Home app. When setting this switch to "on", the Sonos speaker leaves the current group (if any) and joins a new group in the house, that is currently playing. When settings this switch to "off", the speaker leaves the current group (if any) and stops playing.
`playerZonePriority` | [] | When the players are used, zones can be in this priority list (by name as shown in the Sonos app) that is used when searching for a group (that is currently playing) to join.
`playerTVPriority` | `false` | When the players are used, a TV input can have priority when searching for a group (that is currently playing) to join.
